...
function bootstrap_team_repository_new()
{
  TEAM=${1}
  TEAM_REPOSITORY="sdlf-$TEAM-team"
  set +e
  OUTPUT=$(aws codecommit create-repository --repository-name "$TEAM_REPOSITORY" 2>&1)
  STATUS=$?
  set -e
...
declare -a REPOSITORIES=("sdlf-datalakeLibrary" "sdlf-pipLibrary")
create_approval_rule "$TEAM_NAME"
for REPOSITORY in "${REPOSITORIES[@]}"
do
  bootstrap_team_repository "$TEAM_NAME" "$REPOSITORY"
done
bootstrap_team_repository_new "$TEAM_NAME"
