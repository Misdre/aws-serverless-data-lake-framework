...
usage () { echo "
    ...
    crossaccount-cicd-roles -d -p -- Deploys crossaccount IAM roles necessary for DevOps CICD pipelines
        ...
        -p -- Name of the AWS profile to use where a SDLF domain will reside
    devops-account -d -p -- Deploys SDLF DevOps/CICD/Tooling resources
        -d -- Comma-delimited list of AWS account ids where SDLF domains are deployed
    ...
    These resources are used to deploy new domains, teams, pipelines and datasets in the provided comma-delimited list of accounts.
...
crossaccount_cicd_roles () { 
    ...
    echo "Deploying SDLF DevOps/Tooling roles in domain accounts" >&2
    ...
    aws --region "$REGION" --profile "$DOMAIN_AWS_PROFILE" ssm update-service-setting --setting-id arn:aws:ssm:"$REGION:$DOMAIN_ACCOUNT_ID":servicesetting/ssm/parameter-store/high-throughput-enabled --setting-value true
